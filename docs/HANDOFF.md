...
- CU-05 está cerrado y archivado. CU activo: `cu-04-extension-bandeja-interna-invitaciones`, 9/9 tareas; pendiente de cierre formal.
...
- CU-04 EXT: Incrementos backend y frontend completados, validación desktop aprobada para accept Editor y reject Lector. La bandeja MUI usa badge/contador, lista segura, acciones por id y revalidación; aceptar muestra el proyecto sin refresh, rechazar no lo agrega. Link temporal conservado. Realtime de entrega se difiere, por lo que una invitación nueva puede requerir recargar/revalidar `/projects`; una mejora futura reutilizará CU-05 sin un segundo Socket.IO. SMTP está fuera de alcance. Sin schema gap, cambio Prisma ni migración; se mantienen 5 migraciones. Gates: frontend 121/121, backend 40/40, `uml-core` 57/57, raíz 218/218; lint, typecheck y build correctos. CU-06 no iniciado.
...
1. Esperar aceptación para archive, commit y push de CU-04 EXT; no iniciar CU-06.
